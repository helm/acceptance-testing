...
# Invalid kubeconfig file.  Should not cause error in the completion.
_completionTests_verifyCompletion "helm --kubeconfig=/invalid/path lis" ""
...
# The tests for fish don't get the filtered list based on the completion prefix, but instead get the full list.
# This is a problem with the test, not the completion.
# No point in running them.  There are other tests for plugin dynamic completion in the common test file
...
set --global TMP_HELM_DIR (mktemp -d $ROBOT_OUTPUT_DIR/helm-acceptance-temp-bin.XXXXXX)

mkdir -p $TMP_HELM_DIR
set --global HELM_DIR (dirname (which helm))
...
$TMP_HELM_DIR/myhelm completion fish --no-descriptions| source
...
# This is how you unalias
functions --erase helm
